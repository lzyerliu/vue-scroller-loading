    ...
    <div class="_v-content" :id="contentId">
      ...
      >
        <span class="spinner-holder">
          ...
          <span class="text" v-if="state == 1" :style="{color: refreshLayerColor}" v-text="releaseText"></span>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      function infiniteTimeFn () {
        clearTimeout(_this.infiniteTimer)
        _this.infiniteTimer = setTimeout(() => {
          // eslint-disable-next-line
          let { left, top, zoom } = _this.scroller.getValues()
    ...
    destroyed () {
      ...
      if (this.infiniteTimer) clearTimeout(this.infiniteTimer)
    },
